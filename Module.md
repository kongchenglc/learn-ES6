...
var m = 1; export m;
```
因为`export`命令规定的是对外的接口（可以理解为对象的引用，传递的是对象的地址。es6中是引用对象，在引入的内容用到的时候就会读取地址中的内容，所以原来地址中的内容变化会引起引用的变化，所以是动态引用。而commonJS中是复制的缓存，不会动态的改变 ），必须与模块内部的变量建立一一对应关系。上面没有提供对外的接口。第一种写法直接输出`1`，第二种写法通过变量`m`，还是直接输出`1`。`1`只是一个值，不是接口。  
